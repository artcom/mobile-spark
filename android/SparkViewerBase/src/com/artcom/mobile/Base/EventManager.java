...
public class EventManager {
	...
	public static boolean dumpEvent(MotionEvent event) {
		if (INSTANCE != null)
			return INSTANCE.dumpTouchEvent(event);
		return false;
	}
	...
	public boolean dumpTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			if (mode == 2) {
				...
				if (fingerDistance > 1
						&& Math.abs(fingerDistance - fingerDistanceStart) > 20) {
					mode = 3;
					break;
				}
	...
	private void downHandler() {
		...
		String myEvent = "<TouchEvent type='down' x='" + startX + "' y='"
				+ startY + "'/>";
		NativeBinding.onEvent(myEvent);
	}
	...
	private void upHandler() {
		...
		String myEvent = "<TouchEvent type='up' x='" + startX + "' y='"
				+ startY + "'/>";
		NativeBinding.onEvent(myEvent);
	}
	...
	private void singleTapHandler() {
		...
		String myEvent = "<TouchEvent type='tap' x='" + startX + "' y='"
				+ startY + "'/>";
		NativeBinding.onEvent(myEvent);
	}
	...
	private void doubleTapHandler() {
		...
		String myEvent = "<TouchEvent type='doubletap' x='" + startX + "' y='"
				+ startY + "'/>";
		NativeBinding.onEvent(myEvent);
	}
	...
	private void longPressedHandler() {
		...
		String myEvent = "<TouchEvent type='longpress' x='" + startX + "' y='"
				+ startY + "'/>";
		NativeBinding.onEvent(myEvent);
	}
	...
	private void panHandler() {
		AC_Log.debug(" ########### pan: " + startX + ", " + startY
				+ " translate: " + dx + ", " + dy);
		String myEvent = "<GestureEvent type='pan' x='" + startX + "' y='"
				+ startY + "' dx='" + dx + "' dy='" + dy + "'/>";
		NativeBinding.onEvent(myEvent);
	}
	...
	private void pinchHandler() {
		AC_Log.debug(" ########### pinch: factor: " + (float) fingerDistance
				/ fingerDistanceStart);
		String myEvent = "<GestureEvent type='pinch' factor='"
				+ (float) (fingerDistance) / fingerDistanceStart + "'/>";
		NativeBinding.onEvent(myEvent);
	}

	// -------------------------------------------------------------------------
	private void rotationHandler() {
		// AC_Log.debug(" ########### rotation: " + event.getX() + ", " +
		// event.getY());
	}
