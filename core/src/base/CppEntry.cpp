...
namespace cppcore {
    ...
    std::string CppEntry::test() {
        MyTestSuite mySuite("CppEntryTest", 0, 0);
        mySuite.run();
        AC_PRINT << "!!!!!!!!!!!!!!!!!!!!! " << mySuite.getFailedTestMessages();
        return mySuite.getFailedTestMessages();//mySuite.returnStatus() == 0;
    }
...
extern "C" {
    JNIEXPORT bool JNICALL Java_com_artcom_mobile_BaseNativeLib_loadSpark(JNIEnv * env, jobject obj,
                                                                          jstring filename);
    JNIEXPORT jstring JNICALL Java_com_artcom_mobile_BaseNativeLib_test(JNIEnv * env, jobject obj);
};
...
JNIEXPORT jstring JNICALL Java_com_artcom_mobile_BaseNativeLib_test(JNIEnv * env, jobject obj) {
    return (env->NewStringUTF(myCppEntry.test().c_str()));
